  ...
  , methods = list(
    ...
    sumLikelihood = function() {
      ...
      for (compound in names(data.sets)) {
        ...
        peaks.d <- peaksLikelihood(compound)
        
        
       
       
        
        logerror(paste(rnd.num, "Peaks D for compound", compound, "is", peaks.d))
        measurementsLikeli <- c(measurementsLikeli, peaks.d)
    ...
    peaksLikelihood = function(compound) {
      ...
      #QUES: why do we use the dnorm function and not other way
      meas <- data.sets[[compound]][,3][indexes]
      
      numerator <- sum((meas-out)^2)
      meas.mean <-  mean(meas)
      denominator <- sum((abs(out-meas.mean) + abs(meas-meas.mean))^2)
