...
PlotTool <- setRefClass(    
  ...
  , methods = list(
    ...
    plotFinalLHSParameterHistogram = function(lik=-100) {
      logdebug("METHOD IN: plotTool$plotLHSParameterHistogram")
      
      parametersNames <- db$getParametersNames()
      minIt <- db$getMinAcceptedIteration()
      maxIt <- db$getMaxAcceptedIteration()
      plotLHSParametersHistogram(parametersNames, maxIt, "final",lik,details)
      
      logdebug("METHOD OUT: plotTool$plotLHSParameterHistogram")
    },
    #
    # Create plots with the density distribution of the parameters
    #
    plotLHSParametersHistogram = function(parametersNames, iteration, filePrefix="", lik=-100, details=T) {
      
      logdebug("METHOD IN: plotTool$plotLHSParametersHistogram")
      folder <- functionName(match.call())
      mkdir(folder)
      
      totalValues <-  db$getParameters(maxIteration=iteration,lik=lik)
      ...
      for (param in parametersNames) {
        ...
        fileName <- paste("hist",lik,filePrefix,param,formatC(iteration, width=7, flag="0", format="fg"),".png",sep="_")
        png(filename=file.path(folder,fileName))
        ...
        if (details) {
          ...
          plot(d, main=paste(param,"Density"))
          apply(HDR, 1, function(row) {
            x1 <- min(which(d$x >= row[1]))
            x2 <- max(which(d$x < row[2]))
            with(d, polygon(x=c(x[c(x1,x1:x2,x2)]), y=c(0,y[x1:x2],0), col="red", border="red"))
        ...
        if (details) {
          ...
          apply(HDR, 1, function(row) {
            segments(col="red", lwd=5, x0=row[1], y0=0, x1=row[2], y1=0)
          })
        }
    ...
    plotFinalParametersHistogram = function(lik=-100, details=T,HDRper=0.75, filePrefix="ALL") {
      ...
      plotParametersHistogram(chains=chains,parametersNames=parametersNames, iteration=maxIt, filePrefix=filePrefix,lik=lik,details=details,HDRper=HDRper, ow=T)
    ...
    plotParametersHistogramByIteration = function() {
      ...
      for (it in seq(minIt+100, maxIt, by=1000)) {
        plotParametersHistogram(chains,parametersNames, it, filePrefix="evolution",-100)
      }
    ...
    #
    # Get the range  of the most frequent values
    #
    getHighestDensityRegions = function(values, ci) {
      logdebug("METHOD IN: plotTool$getHighestDensityRegions")
      
      hist.obj <- hist(values, breaks=10, plot=F)
      valid.freq <- quantile(hist.obj$counts, ci, na.rm = T)
      break.range <- hist.obj$breaks[2] - hist.obj$breaks[1]
      ini.ranges <- hist.obj$breaks[which(hist.obj$counts > valid.freq)]
      end.ranges <- ini.ranges + break.range
      regions <- data.frame(ini=ini.ranges, end=end.ranges)
          
      logdebug("METHOD OUT: plotTool$getHighestDensityRegions")
      
      return(regions)
      
    },
    getHighestDensityInterval = function(values, ci) {
    ...
    # Create plots with the density distribution of the parameters
    #
    plotParametersBoxPlot = function(lik=-100, filePrefix="") {
      ...
      totalValues <-  db$getParameters(maxIteration=maxIt,lik=lik)
      
      totalValues <- totalValues[,c(-1)]
      loginfo(capture.output(head(totalValues)))
      ...
      filename <- paste0("plotBoxPlot",filePrefix,".png")
...
#    g <- g + geom_boxplot(data=melt(totalValues1c), aes(x=variable,y=value), fill=3, color=3, alpha=0.1)
 ...
 #   g <- g + geom_boxplot(data=melt(totalValues5a), aes(x=variable,y=value), color=1, alpha=0.1)
    ...
    scriptPosteriorsSD = function() {
      ...
      loginfo("StandardDeviations")
      loginfo(capture.output(totalValues))
      loginfo("StandardDeviationsPosteriors")
      loginfo(capture.output(priors))
    ...
    # Create plots with the density distribution of the parameters
    #
    plotParametersHistogram = function(chains, parametersNames, iteration, filePrefix="", lik=-100, details=T, HDRper=0.75, ow=F) {
      ...
      for (id in chains) {
        values[[id]] <- db$getParameters(id,iteration,lik)
      }
      ...
      fileName <- paste("hist",filePrefix,formatC(iteration, width=7, flag="0", format="fg"),lik,"_",HDRper,".png",sep="_")
      
      params.num <- length(parametersNames)
      png(filename=file.path(folder,fileName), width=1000, height=1700)
      logdebug(paste("Columns:",round(params.num/9,0)))
      par(mfrow=c(9,round(params.num/9,0)))
      ...
      for (param in parametersNames) {
        ...
        param.name <- gsub("siteparameter","",param)
        param.name <- gsub("value","",param.name)
        logdebug(paste("Parameter: ",param.name))
       # layout(rbind(1,2), heights=c(9,1))
        
        #TODO: set better the ymax
        x <- totalValues[[param]]
        #HDI <- getHighestDensityInterval(x, HDRper)
        HDR <- getHighestDensityRegions(x, HDRper)
       # logerror(paste(param,'Q1:',quantile(x,0.25),'Q2:',quantile(x,0.75),'mean:',mean(x),'median:',median(x),'sd:',sd(x)))
        max.val <- 0
        for (id in chains) {
          max.val.loc <- max(density(values[[id]][[param]])$y)
          if (max.val.loc > max.val) {
       ...
       if (details) {
         logdebug("hist!")
         par(mar=c(1,1,1,1))
         hist(x, ylim=c(0,max.val), breaks=50, col="white", xlab=paste(param,"values"),
                main=paste(param,"Histogram"), prob=TRUE)  # prob=TRUE for probabilities not counts
       } else {
         logdebug("Plot!")
         par(mar=c(1,1,1,1))
         plot(d, main=param.name, xlab="", ylab="", xaxt="n", yaxt="n")
        # x1 <- min(d$x)
        # x2 <- max(d$x)
        # with(d, polygon(x=c(x[c(x1,x1:x2,x2)]), y=c(0,y[x1:x2],0), col="#d3d3d3", border="#d3d3d3"))
        
         #FOR HDI
         #x1 <- min(which(d$x >= HDI[1]))
         #x2 <- max(which(d$x < HDI[2]))
         #with(d, polygon(x=c(x[c(x1,x1:x2,x2)]), y=c(0,y[x1:x2],0), col="#d3d3d3", border="#d3d3d3"))
         
        # x1 <- min(which(d$x >= min(HDR$ini)))
        # x2 <- max(which(d$x < max(HDR$end)))
        # with(d, polygon(x=c(x[c(x1,x1:x2,x2)]), y=c(0,y[x1:x2],0), col="#d3d3d3", border="#d3d3d3"))
    ...
    plotOutputDBUncertainties = function(out.db,filter="0",title="ALL") {
      ...
      outData$date <- strptime(paste(outData$year, outData$julianday), "%Y %j")
      loginfo(capture.output(head(outData)))
      ...
      for (compound in compounds) {
        ...
        for (site in names(sites)) {
          ...
          #  loginfo(head(stats.data))
          # coefficient.determination.r2(out.data$DWbudkgDWm2, )
          logdebug("MANAGEMENT")
          logdebug(capture.output(mana))#mana[mana$type == "fertilize",]$date
          #loginfo(capture.output(match))
        #  loginfo(paste("compo",compound))
          out.name <- match[[compound]][[1]]
         # loginfo(out.name)
       #   loginfo(class(out.name))
          out.name <- out.db$rmSym(out.name)
        #  loginfo(out.name)
       #dNn2oemiskgNha1
       #stat_summary(fun.data = "iqr", geom="ribbon") +
          g <- ggplot(outData, aes_string(x="date", y=out.name)) +  stat_summary(fun.y = getmode, geom="line", colour="pink")  + stat_summary(fun.y = median, geom="line", colour="blue")  + stat_summary(fun.y=quan95, geom="line", colour="green") + stat_summary(fun.y=quan05, geom="line", colour="green") #+ stat_summary(fun.y = mean, fun.ymin = min, fun.ymax = max, colour="grey") 
       ...
       if (sum(is.na(mana)) == 0) {   
         ...
         # g <- g + geom_vline(xintercept=as.numeric(mana[mana$type == "plant",]$date), colour="blue", linetype="dotted")   
          ...
          filename <- paste0("plotUnc",site,compound,title,"filter",filter,"plotsummary_05_95.png")
          ggsave(file.path(folder,filename), width=15, height=5)
       
       if (FALSE) {
       meas$ymin <- meas$value - meas$sd
       meas$ymax <- meas$value + meas$sd
       y.lim <- max(meas$value, na.rm=T)
       #if (is.na(y.lim)) 
       #{ y.lim <- max(meas$value, na.rm=T)}
       max.lik <- max(outData$likelihood)
       min.lik <- min(outData$likelihood)
       liks <- unique(outData$likelihood)
       lik10 <- quantile(liks,0.1)
       lik25 <- quantile(liks,0.25)
       lik50 <- quantile(liks,0.5)
       lik75 <- quantile(liks,0.75)
       lik85 <- quantile(liks,0.85)
       lik90 <- quantile(liks,0.90)
       lik95 <- quantile(liks,0.95)
       lik99 <- quantile(liks,0.99)

       g <- ggplot() + ylim(c(0,y.lim)) + theme_bw() 
       g <- g + geom_line(data=outData[outData$likelihood <= lik75,], aes_string(x="date", y=out.name, group="likelihood",alpha=0.01), color="blue")
       g <- g + geom_line(data=outData[outData$likelihood > lik75 & outData$likelihood <= lik85,], aes_string(x="date", y=out.name, group="likelihood",alpha=0.01), color="blue")
       g <- g + geom_line(data=outData[outData$likelihood > lik85 & outData$likelihood <= lik90,], aes_string(x="date", y=out.name, group="likelihood",alpha=0.01), color="blue")
       g <- g + geom_line(data=outData[outData$likelihood > lik90 & outData$likelihood <= lik95,], aes_string(x="date", y=out.name, group="likelihood",alpha=0.05), color="blue")
       g <- g + geom_line(data=outData[outData$likelihood > lik95 & outData$likelihood <= lik99,], aes_string(x="date", y=out.name, group="likelihood",alpha=0.05), color="blue")
       g <- g + geom_line(data=outData[outData$likelihood > lik99,], aes_string(x="date", y=out.name, group="likelihood",alpha=0.15), color="blue")
       g <- g + geom_point(data=meas, aes(date,value), color="red", size=1, alpha=0.5)
       g <- g + geom_errorbar(data=meas, aes(x=date,ymin=ymin, ymax=ymax), color="red", size=0.5, alpha=0.15)
       ggsave(file=file.path(folder,paste0(compound,"DIRECT.png")), width=10, height=5)
