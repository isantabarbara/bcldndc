...
Main <- setRefClass(
  ...
  , methods = list(
    ...
    #
    # Initialize the parameter generator and generate the first set of parameters
    #
    ...
    insertParameters = function(posterior, db) {
      ...
      if (posterior$iteration > config$mcmc$gelmanBurnPhaseIterations) {
    ...
    calibrate = function() {
     ...
     #mpi$iniPosteriors(startingParameters)
      ...
      gelmanRubin <- GelmanRubin(parametersNames = parametersNames)
      
      while (!gelmanRubin$reachTheEnd(db)) {
