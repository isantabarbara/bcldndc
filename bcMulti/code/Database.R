...
Database <- setRefClass(    
  ...
  , methods = list(
    ...
    # return data.frame
    #
    getParameters = function(chain=NULL, maxIteration=NULL, lik=-100) {
      
      logdebug("METHOD IN: database$getParameters")
     
      whereChain <- paste0(" WHERE likelihood > ", lik)
      if (is.null(chain) == FALSE) {
        whereChain <- paste(" WHERE likelihood > ", lik, " AND chain IN (", chain, ")",sep="")
        ...
        whereChain <- paste(" WHERE likelihood > ", lik, " AND iteration <= ", maxIteration, sep="")
      }
	...
	#
	# Select the quantile for the likelihood
	#
	selectQLikelihood = function(Q=0.5) {
      sqlCmd <- paste("SELECT likelihood FROM Parameters")
