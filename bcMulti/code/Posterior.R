...
Posterior <- setRefClass(    
  ...
  , methods = list(
    initialize = function(..., chain=0, acceptedCount=0, bestCount=0,isBest=F, iteration=0,lastParameters=data.frame(),
                          acceptanceRate=100.0, lastValue=1e-90, candidateValue=-Inf, bestValue=-Inf, randomWorkaround = 0) {
   ...
   #     validatePosteriorValue(newValue)
    #  }
      u_rand <- randomWorkaround
   #
   # Heuristics needs to be positive and as higher as possible
   #
      metropolis.ratio <- newValue/lastValue
      
      loginfo(paste("Random acceptance. Metropolis Ratio / u_rand:", metropolis.ratio, ">", u_rand))
      if (u_rand <= metropolis.ratio) {

        logdebug(paste(metropolis.ratio, ">", u_rand,"Change posterior for chain", chain))
    ...
    #TODO: esto que hace aqui? mejor borrarlo?
