  ...
  , methods = list(
    ...
    sumLikelihood = function() {
      ...
      for (compound in names(data.sets)) {
        ...
        likeli <- lehuger(compound, log=TRUE)
        ...
        measurementsLikeli <- c(measurementsLikeli, likeli.mean)
      ...
      totalLikelihood <- 1/mean(measurementsLikeli)
    ...
    lehuger = function(compound, log=FALSE) {
      logdebug(paste("METHOD IN: likelihood$lehuger: -Compound:", compound, sep=" "))
      
      lehuger.lik <- -0.5*((data.sets[[compound]][,3]-data.sets[[compound]][,5])/data.sets[[compound]][,4])^2 - 0.9189385 - log(data.sets[[compound]][,4])
      
      if (log) {
        lehuger.lik <- log(lehuger.lik)
      }
      logdebug(paste("METHOD OUT: likelihood$lehuger: -Compound:", compound, sep=" "))
