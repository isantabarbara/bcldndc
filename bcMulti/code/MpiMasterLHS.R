  ...
  , methods = list(
    ...
    initialize = function(..., feeder=NULL, 
      ...
      callSuper(..., feeder=feeder, 
                ...
                prior = prior,
                output.db = output.db,
                output.db.created = output.db.created)
    },
    #
    ...
    executeParallelTasks = function(tasks) {
      ...
      for (taskNumber in 1:length(tasks)) {
        
        task <- tasks[[taskNumber]]
        task$parameters <- getCandidateParameters(task$chain)
        message <- task$process(posteriors[[1]]$iteration)
    ...
    changeAcceptanceRandomNumber = function() {
      for (i in 1:config$parallelThreads) {
        ...
        rnd.num <- get.workaround.rnd.number(posteriors[[1]]$iteration)
        #TODO: this depends of the likelihood values. Very similar likelihoods will give us a uniform posteriors
        # rnd.num <- sample(seq(from=0.1, to=0.99, by=0.1), size=1, replace=T, prob=c(0.05,0.05,0.05,0.05,0.1,0.4,0.4,0.6,0.9))
        #rnd.num <- sample(seq(from=0.1, to=1, by=0.1), size=1, replace=T, prob=c(0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,1))
        # rnd.num <- runif(1)
    ...
    moveParameters = function() {
      for (chain in 1:config$parallelThreads) {
        ...
        posteriors[[chain]]$candidateParameters <<- parametersGenerator$newRandomParameters(posteriors[[chain]]$iteration)
      }
    },
    ...
    updatePosteriors = function(mixed.results, results) {
      ...
      for (chain in 1:config$parallelThreads) {
        ...
        tryCatch({
          ...
          if (accepted) {
            saveOutput(posteriors[[chain]]$iteration, posteriors[[chain]]$isBest, results[[chain]])
            saveLikelihoodSummary(results[[chain]])
          } else {
            saveLikelihoodSummary(results[[chain]])
            loginfo(paste("Iteration",posteriors[[chain]]$iteration,"chain",chain,"Output Not Accepted So No Saved"))
          }
          
        }, error = function(e) {
          logerror(paste("Error saving outputs:",e))
        })
      }
    },
    ...
    saveLikelihoodSummary = function(result) {
      output.db$writeLikelihoodSummary(result$likesummary)
    },
    #
    # Save output in database
    #
    saveOutput = function(iteration, isBest=F, result) {
      
      output.save.step <- 1
      
      if (((iteration %% output.save.step) == 0) | isBest) {
        if (result$ok) {
